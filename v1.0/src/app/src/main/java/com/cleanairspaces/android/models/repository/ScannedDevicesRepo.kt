...
@Inject constructor(
                    ...
                    response.code() == 200 -> {
                        ...
                        try {
                            if (responseBody == null || responseBody.payload.isNullOrBlank()) {
                                ...
                                unEncryptLocationDetailsPayload(payload = responseBody.payload, lTime = responseBody.ltime, resultListener = getMyLocationDataListener)
                            }
                        } catch (e: Exception) {
                            MyLogger.logThis(
                                    TAG,
    ...
    private fun unEncryptLocationDetailsPayload(payload: String, lTime: String, resultListener: AsyncResultListener) {
        try {
            ...
            val locationDetails = Gson().fromJson(unEncJson.toString(), MyLocationDetails::class.java)
            ...
            coroutineScope.launch(Dispatchers.IO) {
                try {
                    ...
                    customerDeviceDataDao.updateIsMyLocation(compId = locationDetails.company_id, locId = locationDetails.location_id)
    ...
    fun fetchDataForMyLocation(compId: String, locId: String, payload: String, ltime: String, getMyLocationDataListener: AsyncResultListener) {
        try {
