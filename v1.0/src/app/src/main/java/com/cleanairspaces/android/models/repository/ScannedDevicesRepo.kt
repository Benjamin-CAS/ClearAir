...
@Inject constructor(
    ...
    private fun getMyLocationResponseCallback(optionalResultWaiter: AsyncResultListener?): Callback<ScannedDeviceQrResponse> {
        return object : Callback<ScannedDeviceQrResponse> {
            override fun onResponse(
                    call: Call<ScannedDeviceQrResponse>,
                    response: Response<ScannedDeviceQrResponse>
    ...
    private fun unEncryptScannedDevicePayload(payload: String, lTime: String?) {
        try {
            ...
            customerDeviceData.device_id = requestedData.get(DEVICE_ID_KEY).asString
            coroutineScope.launch(Dispatchers.IO) {
                //check if user has marked this as their location
                val isFound = customerDeviceDataDao.checkIfIsMyLocation(
                        deviceId =  customerDeviceData.device_id
                )
    ...
    fun fetchDataForMyLocation(
        ...
        try {
            ...
            response.enqueue(getMyLocationResponseCallback(optionalResultWaiter = getMyLocationDataListener))
        } catch (e: Exception) {
            MyLogger.logThis(
                    TAG,
