...
@Inject constructor(
    ...
    fun getADeviceFlow(compId: String, locId: String) =
            locDataFromQrDao.getOutDoorLocDeviceFlow(companyId = compId, locationId = locId)

    fun getADeviceFlowByMonitorId(monitorId: String) =
            locDataFromQrDao.getAMonitorDeviceFlow(monitorId = monitorId)

    fun getAnIndoorLocationById(compId: Int) = locDataFromQrDao.getAnIndoorLocation(compId)
    ...
    private fun getMyIndoorLocationResponseCallback(
            optionalResultWaiter: AsyncResultListener?,
    ): Callback<ScannedDeviceQrResponse> {
        return object : Callback<ScannedDeviceQrResponse> {
            override fun onResponse(
                    ...
                    response.code() == 200 -> {
                        ...
                        try {
                            if (responseBody == null || responseBody.payload.isNullOrBlank()) {
                                MyLogger.logThis(
                                        TAG,
                                        "getMyIndoorLocationResponseCallback()",
                                        "returned a null body"
                                )
                                optionalResultWaiter?.onAsyncComplete(isSuccess = false)
                            } else {
                                ...
                                val lTime = responseBody.ltime ?: "0"
                                unEncryptIndoorLocationDetailsPayload(
                                        payload = responseBody.payload,
                                        lTime = lTime,
                                        resultListener = optionalResultWaiter
                                )
    ...
    fun fetchIndoorLocationDetails(
        ...
        try {
            ...
            val response =
                    qrScannedItemsApiService.fetchInDoorLocationDetails(
                            data = data,
                    )
            ...
            recentlyFetchedDeviceData.add(data)
            data.addProperty(IS_INDOOR_LOC_KEY, true)
            response.enqueue(
                    getMyIndoorLocationResponseCallback(
    ...
    fun fetchLocationDetails(
        ...
        try {
            ...
            data.addProperty(PAYLOAD_KEY, payload)
            val response =
                    qrScannedItemsApiService.fetchDetailsForMyLocation(
                            data = data,
                            method = DEVICE_INFO_METHOD
                    )
    ...
    private fun unEncryptIndoorLocationDetailsPayload(
            payload: String,
            lTime: String,
        ...
        try {
            ...
            val unEncJson = JSONArray(unEncryptedPayload).get(0)
            MyLogger.logThis(
                    TAG,
                    "unEncryptIndoorLocationDetailsPayload(payload: , lTime  : $lTime)",
                    "un encrypted result $unEncJson"
            )
    ...
    companion object {
        ...
        private const val IS_INDOOR_LOC_KEY = "is_indoor_location"
    }
}