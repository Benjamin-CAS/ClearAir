...
interface DeviceDetailsDao {
    ...
    @Query("SELECT * FROM devices_data WHERE for_watched_location_tag =:locationsTag ORDER BY updated_on DESC")
    fun observeDevicesForLocation(locationsTag: String): Flow<List<DevicesDetails>>

    @Query("SELECT * FROM devices_data WHERE watch_device =:watchLocation ORDER BY dev_name")
